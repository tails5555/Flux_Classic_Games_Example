...
public class ConsoleServiceImpl implements ConsoleService{
    ...
    private ConsoleVO integrityConfirm(Console console){
        if (console.getCompany() != null) {
            ...
            if(!curCompany.equals(newCompany)) {
                ...
                return consoleRepository.save(console)
                        .map(c -> ConsoleVO.builtToConsoleVO(c, newCompany)).block();
            }
        }
        return ConsoleVO.builtToConsoleVO(console, console.getCompany());
    }
    ...
    public Mono<ConsoleVO> create(ConsoleForm consoleForm){
        ...
        return consoleRepository.insert(console).map(c -> ConsoleVO.builtToConsoleVO(c, company));
    }
    ...
    public Mono<ConsoleVO> update(String id, ConsoleForm consoleForm){
        ...
        return consoleRepository.findById(id)
                .flatMap(console -> {
                    ...
                    if(!company.equals(new Company()))
                        console.setCompany(company);
                    return consoleRepository.save(console).map(c -> ConsoleVO.builtToConsoleVO(c, company));
                });
    }
